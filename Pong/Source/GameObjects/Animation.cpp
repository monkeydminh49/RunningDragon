...
void Animation::Update(float deltaTime, int start, int end)
{
	/*if ((m_currentFrame.x == (m_frameNum.x - 1)) && m_modeStopAtEndFrame) {
		return;
	}*/
	if (m_currentFrame.x < start || m_currentFrame.x > end) m_currentFrame.x = start;

	if ((m_currentFrame.x == end) && m_modeStopAtEndFrame) {
		return;
	}
	m_currentTime += deltaTime;
	if (m_currentTime >= m_frameTime) {
		m_currentFrame.x++;
		if (m_currentFrame.x == m_frameNum.x || m_currentFrame.x >= end) {
			m_currentFrame.x = start;
		}
		CalculateRectUV();
		ApplyRect();
